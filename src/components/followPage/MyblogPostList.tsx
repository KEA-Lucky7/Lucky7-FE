...
const FollowPostList: React.FC = () => {
    const [currentPage, setCurrentPage] = useState<number>(1);
    const [selectedTab, setSelectedTab] = useState<'picture' | 'article'>('picture'); // Default selected tab is 'article'
        ...
        <S.PostListContainer>
            {currentPosts.map((post) => (
                <S.PictureListBox key={post.id}>
                    ...
                    <S.PictureBox>
                        <img src={Titlebackground} alt={post.title} style={{ width: '100%', height: '100%' }} />
                    </S.PictureBox>
                </S.PictureListBox>
            ))}
        </S.PostListContainer>
